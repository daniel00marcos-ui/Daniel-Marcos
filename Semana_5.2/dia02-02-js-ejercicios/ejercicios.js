...
function sumarArray(arr = []) {
  return arr.reduce(function (total, numero) {
    return total + numero
  }, 0)
}
...
console.log('Ejercicio 6: Es palíndromo')
...
function elementosEnComun(arr1 = [], arr2 = []) {
  return [...new Set(arr1.filter(function (elemento) {
...
function contarPalabras(frase = '') {
  ...
  return fraseLimpia.split(/\s+/).length
}
...
function reemplazarPalabra(frase = '', buscar = '', reemplazar = '') {
  ...
  return frase.replaceAll(buscar, reemplazar)
}
...
function caracterFrecuente(texto = '') {
  const caracteres = texto.replaceAll(' ', '')
  const contador = {}
  ...
  for (const caracter of caracteres) {
    ...
    if (contador[caracter] > cantidadMayor) {
      cantidadMayor = contador[caracter]
...
function contarOcurrencias(frase = '') {
  ...
  const palabras = fraseLimpia.split(/\s+/)
...
console.log(contarOcurrencias('a b a b c'))
// { a: 2, b: 2, c: 1 }

console.log(contarOcurrencias(''))
// {}

console.log(contarOcurrencias('hola hola mundo'))
// { hola: 2, mundo: 1 }
...
function obtenerLongitudes(texto = '') {
  ...
  return textoLimpio.split(/\s+/).map(function (palabra) {
...
function promedioArray(arr = []) {
  ...
  const suma = arr.reduce(function (total, numero) {
    return total + numero
  }, 0)
...
function textoALista(texto = '') {
  ...
  return texto.split(',').map(function (elemento) {
